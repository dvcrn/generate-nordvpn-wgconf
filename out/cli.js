...
__export(cli_exports, {
  generateConfigsForCountry: () => generateConfigsForCountry
});
...
var Y18N = class {
  ...
  _processWriteQueue() {
    ...
    shim.fs.writeFile(languageFile, serializedLocale, "utf-8", function(err2) {
  ...
  _readLocaleFile() {
    ...
    try {
      ...
      if (err2 instanceof SyntaxError) {
        err2.message = "syntax error in " + languageFile;
      }
      if (err2.code === "ENOENT")
        localeLookup = {};
      else
...
function usage(yargs, shim3) {
  ...
  self.fail = function fail(msg, err2) {
    ...
    if (fails.length) {
      ...
      err2 = err2 || new YError(msg);
      if (yargs.getExitProcess()) {
        return yargs.exit(1);
      } else if (yargs.getInternalMethods().hasParseCallback()) {
        return yargs.exit(1, err2);
      } else {
        throw err2;
      }
    }
...
var YargsInstance = class {
  ...
  check(f, global) {
    ...
    this.middleware((argv2, _yargs) => {
      return maybeAsyncResult(() => {
        ...
        __classPrivateFieldGet(this, _YargsInstance_usage, "f").fail(err2.message ? err2.message : err2.toString(), err2);
        return argv2;
      });
    }, false, global);
    return this;
  ...
  exit(code, err2) {
    ...
    __classPrivateFieldSet(this, _YargsInstance_exitError, err2, "f");
  ...
  parse(args, shortCircuit, _parseFn) {
    ...
    if (isPromise(parsed)) {
      return parsed.then((argv2) => {
        ...
        if (__classPrivateFieldGet(this, _YargsInstance_parseFn, "f")) {
          __classPrivateFieldGet(this, _YargsInstance_parseFn, "f")(err2, this.parsed.argv, __classPrivateFieldGet(this, _YargsInstance_output, "f"));
        }
        throw err2;
      }).finally(() => {
        this[kUnfreeze]();
  ...
  [kRunYargsParserAndExecuteCommands](args, shortCircuit, calledFromCommand, commandIndex = 0, helpOnly = false) {
    ...
    try {
      ...
      if (requestCompletions) {
        ...
        __classPrivateFieldGet(this, _YargsInstance_completion, "f").getCompletion(completionArgs, (err2, completions) => {
          if (err2)
            throw new YError(err2.message);
      ...
      if (err2 instanceof YError)
        __classPrivateFieldGet(this, _YargsInstance_usage, "f").fail(err2.message, err2);
      else
        throw err2;
    }
...
// configUtils.ts
var generateWireguardConfig = (privateKey, publicKey, dnsServer, endpoint) => {
  return `[Interface]
...
var extractWireguardConfig = (server) => {
...
// api.ts
var fetchNordCountries = async () => {
  const res = await fetch(`https://api.nordvpn.com/v1/countries`);
  return await res.json();
};
var fetchRecommendedServers = async (countryId, amount = 1) => {
...
var getCountryByCode = async (code) => {
...
var extractNordConfig = (accountId) => {
  return new Promise((resolve5, reject) => {
...
// cli.ts
var argv = yargs_default(hideBin(process.argv)).option("country", {
  alias: "c",
  type: "string",
  description: "Country code of the country to generate the config for, eg 'US'"
...
var generateConfigsForCountry = async (country, argv2) => {
  console.log(`Generating ${argv2.amount} configs for ${country.name}`);
  let nordConfig;
  if (argv2["wireguard-privatekey"]) {
    nordConfig = {
      private_key: argv2["wireguard-privatekey"],
      dns_servers: [argv2["dns-server"]]
    };
  } else if (argv2["nordvpn-accountid"]) {
    console.log(
      "no --pk specified, trying to extract from keychain with id ",
      argv2["nordvpn-accountid"]
    );
    nordConfig = await extractNordConfig(argv2["nordvpn-accountid"]);
  } else {
    console.log(
  ...
  recommendedServers.forEach((recommendedServer) => {
    try {
      ...
      const outputDir = import_path5.default.resolve(argv2.outdir);
      if (!import_fs5.default.existsSync(outputDir)) {
        import_fs5.default.mkdirSync(outputDir, { recursive: true });
      }
      const outPath = import_path5.default.join(
        outputDir,
        `NordVPN ${recommendedServer.name}.conf`
      );
      import_fs5.default.writeFileSync(outPath, wgConfigString);
